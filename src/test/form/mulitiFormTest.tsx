...
export const MultiFormTest: FormStory = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Tom has 1000000 salary', async () => {
      ...
      await expect(canvas.getByTestId('name').getAttribute('value')).toEqual(
        'Tom'
      );

      await userEvent.click(canvas.getByText('Set Salary'));

      await expect(canvas.getByTestId('salary').getAttribute('value')).toEqual(
        '1000000'
      );

      await expect(canvas.getByText('恭喜，达成百万年薪')).toBeInTheDocument();
    });
