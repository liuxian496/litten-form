...
export const ValidationByStepTest: FormStory = {
  parameters: {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      'Click Save button with empty fields, Then "Name" validation errors to be displayed',
      async () => {
        await userEvent.click(saveButton);

        const nameError = await canvas.findByText('This field is required.');
        const salaryError = await canvas.queryByText(
    ...
    await step(
      'Type "Tom" in Name Field, Click save button, Then "Salary" field validation error to be displayed',
      async () => {
        await userEvent.type(nameInput, 'Tom');
        await userEvent.click(saveButton);

        const nameError = await canvas.queryByText('This field is required.');
    ...
    await step(
      'Type valid salary, Click save button, Then no validation error to be displayed',
      async () => {
